...
`,Cr=ae({"vira-icon-color":"currentColor"}),Hc=ae({"vira-icon-stroke-color":Cr["vira-icon-color"].value,"vira-icon-fill-color":Cr["vira-icon-color"].value}),pt={...Cr,...Hc},B=et()({tagName:"vira-icon",hostClasses:{"vira-icon-fit-container":({inputs:e})=>!!e.fitContainer},styles:({hostClasses:e})=>w`
        :host {
...
`;const{defineElement:j,defineElementNoInputs:eu}=Go(),Z=j()({tagName:"book-route-link",cssVars:{"book-route-link-anchor-padding":""},styles:({cssVars:e})=>w`
    ...
    `,renderCallback({inputs:e,dispatch:t,events:r}){return Object.entries(e.config).length?Object.entries(e.config).map(([n,o],a)=>{if(o.controlType===_.Hidden)return"";const s=Vc(e.currentValues[n],o,i=>{const l=Lt(e.fullUrlBreadcrumbs,"array")?e.fullUrlBreadcrumbs:e.fullUrlBreadcrumbs[n];if(!l)throw new Error(`Failed to find breadcrumbs from given control name: '${n}'`);t(new r.controlValueChange({fullUrlBreadcrumbs:l,newValues:{...e.currentValues,[n]:i}}))});return g`
    ...
    `,initCallback({host:e}){setTimeout(()=>{vo(e)},1e3)},cleanupCallback({state:e,updateState:t}){e.router&&(e.router.removeAllRouteListeners(),t({router:void 0}))},renderCallback:({state:e,inputs:t,host:r,updateState:n,dispatch:o,events:a})=>{var i,l,c,u;try{let d=function(C){e.router?e.router.setRoutes(C):n({currentRoute:{...e.currentRoute,...C}}),t.elementBookRoutePaths&&!Ne(t.elementBookRoutePaths,e.currentRoute.paths)&&o(new a.pathUpdate(C.paths??[]))};var s=d;if(t.elementBookRoutePaths&&!Ne(t.elementBookRoutePaths,e.currentRoute.paths)&&d({paths:t.elementBookRoutePaths}),(i=t.internalRouterConfig)!=null&&i.useInternalRouter&&!e.router){const C=tl(t.internalRouterConfig.basePath);n({router:C}),C.addRouteListener(!0,S=>{n({currentRoute:S})})}else!((l=t.internalRouterConfig)!=null&&l.useInternalRouter)&&e.router&&e.router.removeAllRouteListeners();const h={themeColor:t.themeColor};if(!Ne(h,(c=e.colors)==null?void 0:c.config)){const C=so(h);n({colors:{config:h,theme:C}}),ol(r,C)}const f=t.debug??!1,p=Ai({entries:t.entries,everythingTitle:t.everythingTitle,everythingDescriptionParagraphs:t.everythingDescriptionParagraphs??[],debug:f});(!e.treeBasedCurrentControls||e.treeBasedCurrentControls.entries!==t.entries||e.treeBasedCurrentControls.globalValues!==t.globalValues)&&n({treeBasedCurrentControls:{entries:t.entries,globalValues:t.globalValues??{},currentControls:sa(p.tree,t.globalValues??{})}});const m=ca(e.currentRoute.paths),x=(m?Ui({flattenedNodes:p.flattenedNodes,searchQuery:m}):void 0)??Wc(p.flattenedNodes,e.currentRoute.paths,d),E=(u=e.treeBasedCurrentControls)==null?void 0:u.currentControls;return E?(t.debug&&console.info({currentControls:E}),g`
                <div
                    ...
                    ${R(We.events.controlValueChange,C=>{if(!e.treeBasedCurrentControls)return;const S=Ni(E,C.detail.fullUrlBreadcrumbs,C.detail.newValues);n({treeBasedCurrentControls:{...e.treeBasedCurrentControls,currentControls:S}})})}
                >
            ...
            `,renderCallback({controls:t}){const r=w`
                ...
                `}}),e({title:"example with error",renderCallback(){return"broken"}}),e({title:"example with error",renderCallback(){return"broken"}})}}),Xa=[Fr,$o(0,Sr),yo,...Array(100).fill(0).map((e,t)=>$o(t+1,yo)),wo,wo,Zc,Sr];console.info({entries:Xa});zt({tagName:"vir-app",styles:w`
        :host {
            display: flex;
            flex-direction: column;
            gap: 24px;
